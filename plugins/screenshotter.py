...
class ScreenShotter(Inject, Plugin):
	...
	def request(self, request):
		if 'saveshot' in request.uri:
			...
			img_file = '{}-{}-{}.png'.format(client, request.headers['host'],
	...
	def get_payload(self):
		return re.sub("SECONDS_GO_HERE", str(self.interval * 1000),
		              open("./core/javascript/screenshot.js", "rb").read())

	def options(self, options):
		options.add_argument("--interval", dest="interval", type=int, metavar="SECONDS", default=10,
		                     help="Interval at which screenshots will be taken (default 10 seconds)")
